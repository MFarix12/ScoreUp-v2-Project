...
function NavigationLinks({
  ...
  return (
    <nav className="space-y-1.5 p-4" aria-label="Sports Technician navigation">
      {navigation.map((item) => {
...
export function TechnicianSidebar({ profileName }: { profileName: string }) {
    ...
    <>
          ...
          <div className="rounded-2xl bg-white/5 p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs font-medium text-red-300">Sports Technician</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <header className="fixed inset-x-0 top-0 z-40 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 shadow-sm backdrop-blur lg:hidden">
        ...
        <button
          ...
          aria-label="Open Sports Technician navigation"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button
            type="button"
            aria-label="Close Sports Technician navigation"
          ...
          <aside className="absolute inset-y-0 left-0 flex w-[min(88vw,320px)] flex-col bg-[#111827] text-white shadow-2xl">
            ...
            <div className="space-y-3 border-t border-white/10 p-4">
              <div className="rounded-2xl bg-white/5 p-4">
                <p className="truncate text-sm font-bold text-white">{profileName}</p>
                <p className="mt-1 text-xs font-semibold text-red-300">Sports Technician</p>
              </div>
